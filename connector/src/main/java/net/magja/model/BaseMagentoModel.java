...
public abstract class BaseMagentoModel<S> implements Serializable {
  ...
  protected static final String MAPPING_FILE_NAME_SUFFIX = "-mapping";
  ...
  /**
   * Load JavaBean property <-> Magento API mapping from .properties file. See
   * *.properties files in src/main/resources/net/magja/model folders.
   */
  protected void loadMappings() {
    String className = getClass().getSimpleName();
    className = className.substring(0, 1).toLowerCase() + className.substring(1);
    className = getClass().getPackage().getName() + "." + className + MAPPING_FILE_NAME_SUFFIX;
        ...
        if (prefix.equals("set")) {
          ...
          if (fld.getType().equals(Class.forName("java.lang.Boolean"))) {
            ...
            if (arg.getClass().getSimpleName().equals("Boolean")) {
              ...
              if (arg.toString().equals("1") || arg.equals("true")) {
                args[0] = new Boolean(true);
              } else if (arg.toString().equals("0") || arg.equals("false")) {
                args[0] = new Boolean(false);
              } else {
                args[0] = new Boolean(arg.toString());
              }
            }

          } else {

            // sometimes the arg could still have in boolean type, so we convert
            // here
            Object arg2;
  ...
  /**
   * Get a specific, in the magento's name format, which is or isn't covered by
   * the concret model
   */
  public Object get(String name) {
    if (name != null) {
      if (!name.trim().equals("")) {
        ...
        // once we have the attribute name, just invoke the proper 'get' for
        // that to invoke the correspondent model's method
        try {
  ...
  public void set(String name, Object value) {
    if (name != null && value != null) {
      ...
      if (!name.equals("")) {
        /*
         * first, we search for a correspondent model attribute for this
         * property if exists, we invoke the setter for this property instead
         * put it on the map
         */
  ...
  /**
   * Serializes the model to representation compatible with Magento API. This
   * method is useful if you pass the object to Magento.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (getClass() != obj.getClass())
      return false;
    BaseMagentoModel<S> other = (BaseMagentoModel<S>) obj;
    if (id == null) {
      if (other.id != null)
        return false;
