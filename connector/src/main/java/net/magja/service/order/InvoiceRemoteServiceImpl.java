...
import net.magja.model.order.InvoiceItem;
...
public class InvoiceRemoteServiceImpl extends GeneralServiceImpl<Invoice> implements InvoiceRemoteService {
  ...
  public void addComment(Invoice invoice, String comment, Boolean email, Boolean includeComment) throws ServiceException {
    try {
      soapClient.callArgs(ResourcePath.SalesOrderInvoiceAddComment,
          new Object[] { invoice.getId(), comment != null ? comment : "", email ? "1" : "0", includeComment ? "1" : "0" });
    } catch (AxisFault e) {
      if (debug)
        e.printStackTrace();
  ...
  public void save(Invoice invoice, String comment, Boolean email, Boolean includeComment) throws ServiceException {
    ...
    try {
      ...
      id = Integer.parseInt((String) soapClient.callArgs(ResourcePath.SalesOrderInvoiceCreate,
          new Object[] { invoice.getOrderNumber(), invoiceItems, comment != null ? comment : "", email ? 1 : 0, includeComment ? 1 : 0 }));
